...
| 13 | Terminal-Bench 2.1 + 4.0 | 40 rows from the Supabase function the page calls (2.1: 22 + 4.0: 18), each with its agent, effort and run date; the 4.0 selector (`4-0-0` on the bare package) was read from the leaderboard page's own dehydrated query state. Supersedes batch 02's 2.1 rows. |
